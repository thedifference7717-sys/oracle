...
async function main() {
  ...
  if (P.alerts.lockDate !== day) {
    ...
    await tg(
      `🔒 <b>PARLAIY LOCKED</b> · ${prettyDate(day)}\n` +
      `Round robin · ${picks.length} picks · ${combos} doubles\n` +
      `$${(+stake).toFixed(2)}/double · $${(combos * stake).toFixed(2)} total · legs ${odds > 0 ? "+" : ""}${odds}\n` +
      `Line: Over 2.5 (H + R + RBI)\n` +
      `➖➖➖➖➖➖➖➖\n${lines}`
    );
  ...
  const tally = () => `Board: 💣 ${cashed}  💀 ${dead}  ⏳ ${picks.length - cashed - dead}  of ${picks.length}`;
  for (const p of picks) {
    ...
    if (s && s.c >= RR_LINE && !P.alerts.hits[p.id]) {
      ...
      await tg(
        `💣 <b>CASHED</b> — ${p.name}\n` +
        `${s.h} H · ${s.r} R · ${s.rbi} RBI  →  ${s.c} (need ${RR_LINE})\n` +
        tally()
      );
      ...
      dead++;
      const line = s ? `${s.h} H · ${s.r} R · ${s.rbi} RBI  →  ${s.c} (need ${RR_LINE})` : "DNP — did not play";
      await tg(
        `💀 <b>DEAD</b> — ${p.name}\n` +
        `${line}\n` +
        tally()
